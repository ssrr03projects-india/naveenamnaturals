...
function OrderDetailContent() {
  ...
  const handleCreateShipment = async () => {
    ...
    try {
      ...
      let addressData = order.address || order.billingAddress;

      // Ensure address is an object if it came as a string
      ...
      const payload = {
        // customer_code and service_type_id are intentionally omitted here
        // so the backend uses its env-configured defaults (CL5527, B2C SMART EXPRESS)
        ...
        weight_unit: "kg",
        weight: "0.5",
        declared_value: displayedTotalPaid,
        num_pieces: "1",
        origin_details: {
          name: "Naveenam Naturals",
          phone: "9994983377",
          address_line_1: "No 17 Sylvan Lodge Colony, Kelly's Road, Kilpauk",
          pincode: "600010",
          city: "Chennai",
          state: "Tamil Nadu",
        },
        destination_details: {
          name: addressData?.name || customerName,
          phone: addressData?.phone || order.customer?.phone || "0000000000",
          address_line_1: addressData?.address
            ? addressData.address.replace(/\n/g, ", ")
            : "Address Missing",
          address_line_2: addressData?.address2 || "",
          pincode: addressData?.postalCode || addressData?.pincode,
          city: addressData?.city,
          state: addressData?.state,
        },
        order_id: order.orderNumber,
        cod_amount: order.paymentMethod === "cod" ? displayedTotalPaid : "0",
      ...
      if (result.success) {
        ...
        // Reload order to get new AWB/status if backend updates it
        // Since backend doesn't automatically update order table with AWB yet (we need to impl that bridge),
        // we might just show success for now. Ideally backend createShipment should update order.trackingNumber
        loadOrder();
      } else {
