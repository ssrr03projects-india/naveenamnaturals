                ...
                >
                    ...
                    <div className="list-size flex items-center gap-2 flex-wrap mt-3">
                        {Array.isArray(data?.sizes) && data.sizes.map((item, index) => (
                            <div
                                className={`size-item w-12 h-12 flex items-center justify-center text-button rounded-full bg-white border border-line ${activeSize === (typeof item === 'object' ? item.size : item) ? 'active' : ''}`}
                                key={index}
                            >
                                {typeof item === 'object' ? item.size : item}
                            </div>
                        ))}
                    </div>
                    <table>
