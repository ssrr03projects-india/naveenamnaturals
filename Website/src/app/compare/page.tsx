...
import { formatProductPrice } from "@/lib/price-utils";
...
const Compare = () => {
  ...
  const handleAddToCart = (productItem: ProductType) => {
    if (!cartState.cartArray.find((item) => item.id === productItem.id)) {
      ...
      updateCart(String(productItem.id), productItem.quantityPurchase, "");
    } else {
      updateCart(String(productItem.id), productItem.quantityPurchase, "");
    }
    openModalCart();
                    ...
                    {compareState.compareArray.map((item, index) => (
                      ...
                      >
                        <div className="h-full flex items-center justify-center capitalize gap-1">
                          {Array.isArray(item.sizes) ? item.sizes.map((size, i) => (
                            <p key={i}>
